...
| ElastiCache, Redis (OSS), Valkey, Memcached, cache cluster, ElastiCache Serverless — node-based **Redis OSS On-Demand verified**; **Serverless is best-effort** (the Serverless scalars are ~21% of the captured total and don't decompose into published ECPU/storage rates — keep them `"0"` unless the user asks for Serverless); only the Redis OSS `EngineType` token is captured (**Valkey / Memcached tokens and Reserved-node `TermType` strings are inferred**); Redis Enterprise Cloud is Marketplace and NOT modeled | `amazonElastiCache` (`estimateFor: amazonElastiCache`) | [`service-modules/elasticache.md`](service-modules/elasticache.md) |
