...
| Valkey on x86 node types (`cache.m5.large` x2, `cache.t3.medium` x1/x2, `cache.r5.large` x2, eu-central-1; `Instance Family` Standard for m5/t3, Memory optimized for r5) | recompute-verified | live SPA 2026-09-24, x86 variant of the reference estimate: all 5 lines to the cent. ElastiCache has no 6th/7th-gen Intel node types in the Price List (no `cache.m6i`/`m7i`/`r6i`/`r7i` `instanceType` values), so x86 tops out at m5/r5/t3 |
