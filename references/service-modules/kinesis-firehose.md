...
| Kinesis Data Streams as the source | recompute-verified | form 0.0.112 has **no `"kds"` value**: its options are `direct` (labelled "Direct PUT or Kinesis Data Stream"), `vended`, `msk`. Use `"direct"`; `<P>-StreamsSourceBilledBytes` and `<P>-BilledBytes` carry the same tiers. Minimal shape in `references/fixtures/amazonKinesisFirehose-kds.json` (5.6 records/s x 25 KB, `recordMultDI "1"`, toggles off, $11.58 eu-central-1) + live SPA 2026-09-24, 42-line multi-account reference estimate (customer engagement, ID withheld) |
| `sourceType` `"vended"` / `"msk"` | inferred | form options only |
