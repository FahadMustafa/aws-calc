...
- **Capture source:** `captures/calculator.aws_new.har` (local capture, not in repo) entry 346 (POST to `https://dnd5zrqcec4or.cloudfront.net/Prod/v2/saveAs`), service key `amazonSimpleQueueService-08629a9d-...`. Saved verbatim to `/tmp/aws_calc_onboard/amazonSimpleQueueService.json` (path was ephemeral; file lost — re-capture needed).
